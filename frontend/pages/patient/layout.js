function getPatientNavItems(activeTab) {
  ...
  return items.map((item) => {
    ...
    return `
      <a href="${item.href}" data-bottom-nav-link="patient" class="card-hover flex min-w-0 flex-1 flex-col items-center justify-center rounded-[16px] px-2 py-2 transition-all duration-200 active:scale-90 ${isActive ? 'bg-primary/10 text-primary shadow-sm' : 'text-on-surface-variant hover:bg-surface-container-low'}">
...
export function createPatientPageShell({ title, activeTab, headerActions = '', headerIcon = 'home', children, bodyClass = 'bg-background' }) {
  return `
    <div class="page-enter min-h-screen ${bodyClass} pb-24 text-on-surface">
...
export function createPatientActionGrid(items) {
  return `
    <section class="grid grid-cols-2 gap-3">
      ${items.map((item) => `
        <a href="${item.href}" class="card-hover glow-surface flex flex-col items-center gap-2 rounded-[20px] border border-outline-variant/30 bg-white p-4 shadow-[0px_4px_20px_rgba(0,0,0,0.04)] transition-transform active:scale-95">
